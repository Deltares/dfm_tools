...
def write_noosfile(filename, pd_data, metadata=None, na_values=None, float_format='%8.3f'):
    ...
    with open(filename,'w') as file_noos:
        pass
    
    with open(filename,'a') as file_noos:
        file_noos.write('# ----------------------------------------------------------------\n')
    ...
    pd_data.to_csv(filename, header=None, index=None, sep='\t', mode='a', date_format='%Y%m%d%H%M', float_format=float_format, na_rep=na_values)
