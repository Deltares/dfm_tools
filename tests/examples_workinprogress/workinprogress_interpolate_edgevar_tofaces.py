...
if 'RMM_dflowfm' in file_nc: #interpolation is slow for many timesteps, so subsetting first
    uds = uds.isel(time=[-1])
...
data_fec = xr.DataArray(uds.grid.face_edge_connectivity,dims=(dimn_faces,dimn_maxfn))
...
edgevar_tofaces_onint = edgevar_tofaces_onint_step2.mean(dim=dimn_maxfn,keep_attrs=True)
print('interpolation with indexer: done')

#update attrs from edge to face
face_attrs = {'location': 'face', 'cell_methods': 'nmesh2d_face: mean'}
edgevar_tofaces_onint = edgevar_tofaces_onint.assign_attrs(face_attrs)
...
if dimn_interface in edgevar_tofaces_onint.dims:
    if 0:
        ...
        # rename int to lay dimension
        edgevar_tofaces = edgevar_tofaces.rename({dimn_interface:dimn_layer})
        # re-assign variable attributes
        edgevar_tofaces = edgevar_tofaces.assign_attrs(edgevar_tofaces_onint.attrs)
    else: #TODO: this only works if nmesh2d_interface does not accidentally has different index set (like depth)
        nlayers = uds.dims[dimn_layer]
        array_shift_half = xr.DataArray(np.arange(0.5,nlayers),dims=dimn_layer)
        edgevar_tofaces = edgevar_tofaces_onint.interp({dimn_interface:array_shift_half})
else:
    edgevar_tofaces = edgevar_tofaces_onint.assign_attrs(edgevar_tofaces_onint.attrs)
