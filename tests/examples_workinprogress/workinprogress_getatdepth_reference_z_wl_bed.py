...
def get_mapdata_atdepth(data_xr_map, depth, reference='z0', varname=None, zlayer_z0_interp=False):
    ...
    depth:
        depth w.r.t. reference level. If reference=='waterlevel', depth>0 returns only nans. If reference=='bedlevel', depth<0 returns only nans. 
    reference:
    ...
    data_xr_map_ondepth = data_xr_map_var.where(bool_valid_wl&bool_valid_bl&bool_mindist).max(dim='nmesh2d_layer',keep_attrs=True) #set all layers but one to nan, followed by an arbitrary reduce (max in this case)
...
for file_nc in file_nc_list:
    ...
    data_frommap_timesel_ondepth = get_mapdata_atdepth(data_xr_map=data_frommap_timesel, depth=depth, 
                                                       reference='bedlevel', zlayer_z0_interp=True)
    fig, ax = plt.subplots()
    ...
    #reference for bottom layer
    fig, ax = plt.subplots()
    data_var = data_frommap_timesel.ffill(dim='nmesh2d_layer').bfill(dim='nmesh2d_layer') #replace nans in toplayer with first non-nan value in layers below (and for bottomlayer with first non-nan value in layers above)
    data_var_sel = data_var.isel(nmesh2d_layer=0) #since nans are now filled, -1 provides the toplayer of the model and 0 the bottomlayer
    pc = data_var_sel['mesh2d_sa1'].ugrid.plot(edgecolor='face',cmap='jet')
    pc.set_clim(clim_sal)
    ax.set_aspect('equal')
    fig.tight_layout()
