...
data_xyn1 = hcdfm.ObservationPointModel(Path(file_xyn)) #TODO: this should raise an error, but it returns an empy list
data_xyn2 = hcdfm.XYNModel(Path(file_xyn)) #TODO: test_obs.xyn should raise an error
data_xyn2_pd = dfmt.pointlike_to_DataFrame(data_xyn2) #works as expected #TODO: add DataFrame_to_xyn() conversion?
data_xyn2.save(file_xyn.replace('_obs.xyn','_out_obs.xyn')) #TODO: "TypeError: serialize() takes 3 positional arguments but 4 were given"
...
data_crs1 = hcdfm.CrossLocModel(Path(file_crs)) #TODO: this should raise an error, but it returns an empty list.
