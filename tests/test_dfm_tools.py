...
def Test_grid_getnetdata_getmapmodeldata_plotnetmapdata(self):
    ...
    from dfm_tools.grid import get_netdata, get_mapmodeldata, plot_netmapdata
    ...
    data_frommap = get_mapmodeldata(file_nc=file_map8, var_values='mesh2d_flowelem_bl', timestep=3, lay=33)#, multipart=False)
    ...
    data_frommap = get_mapmodeldata(file_nc=file_map8, var_values='mesh2d_s1', timestep=3, lay=33)#, multipart=False)
    data_frommap_flat = data_frommap.flatten()
    fig, ax = plt.subplots()
    pc = plot_netmapdata(ugrid_all.verts, values=data_frommap_flat, ax=None, linewidth=0.5, cmap="jet")
    pc.set_clim([-0.5,1])
...
def Test_maplora(self):
    ...
    """
    print('plot grid and values from mapdata (constantvalue, 1 dim)')
    data_frommap = get_mapmodeldata(file_nc=file_maplora, var_values='mesh2d_FColi', timestep=3, lay=33)#, multipart=False)
    data_frommap_flat = data_frommap.flatten()
    fig, ax = plt.subplots()
